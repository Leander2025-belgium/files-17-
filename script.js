...
const state = {
  ...
  earth3d: { active:false, cesiumLoaded:false, viewer:null, imageryLayer:null, frames:[], index:0, layer:'precip', timer:null, searchTimer:null, eco:false },
...
$('#earth3dBtn')?.addEventListener('click', openEarth3d);
...
$('#chip3dEarth')?.addEventListener('click', openEarth3d);
...
function likelyWeakDevice(){
  const mem = navigator.deviceMemory || 4;
  const cores = navigator.hardwareConcurrency || 4;
  const small = Math.min(screen.width, screen.height) <= 430;
  return mem <= 3 || cores <= 4 || small;
}
...
async function ensureCesium(){
  if(state.earth3d.cesiumLoaded && window.Cesium) return true;
  window.CESIUM_BASE_URL = CESIUM_BASE;
  loadCssOnce(CESIUM_BASE + 'Widgets/widgets.css');
  await loadScriptOnce(CESIUM_BASE + 'Cesium.js');
  state.earth3d.cesiumLoaded = Boolean(window.Cesium);
  return state.earth3d.cesiumLoaded;
}

async function openEarth3d(){
  state.earth3d.active = true;
  lockPageScroll();
  $('#earth3dScreen')?.classList.add('active');
  $('#earth3dScreen')?.setAttribute('aria-hidden','false');
  const weak = likelyWeakDevice();
  state.earth3d.eco = weak;
  if(navigator.getBattery){
    navigator.getBattery().then(b=>{
      if(b.level < .25 && !b.charging){
        state.earth3d.eco = true;
        if($('#earth3dEco')) $('#earth3dEco').checked = true;
        if(state.earth3d.viewer){
          state.earth3d.viewer.resolutionScale = .58;
          state.earth3d.viewer.targetFrameRate = 18;
          state.earth3d.viewer.scene.globe.maximumScreenSpaceError = 6;
        }
      }
    }).catch(()=>undefined);
  }
  if($('#earth3dEco')) $('#earth3dEco').checked = state.earth3d.eco;
  $('#earth3dFallback')?.classList.toggle('hidden', !weak);
  updateEarth3dLegend('3D Aarde laden...', 'CesiumJS wordt pas nu geladen om de gewone app snel te houden.');
  try{
    await ensureCesium();
    initEarth3dViewer();
    wireEarth3dControls();
    await refreshEarth3dFrames();
    flyEarth3dTo(state.loc.lat, state.loc.lon, 2400000);
    setTimeout(()=>state.earth3d.viewer?.resize(), 80);
  }catch(e){
    console.warn('3D Aarde mislukt:', e?.message || e);
    updateEarth3dLegend('3D niet beschikbaar', 'Gebruik de 2D-radar. De app blijft verder werken.');
    $('#earth3dFallback')?.classList.remove('hidden');
  }
}

function closeEarth3d(){
  stopEarth3dPlayback();
  state.earth3d.active = false;
  $('#earth3dScreen')?.classList.remove('active');
  $('#earth3dScreen')?.setAttribute('aria-hidden','true');
  if(state.earth3d.viewer){
    state.earth3d.viewer.scene.requestRenderMode = true;
    state.earth3d.viewer.scene.maximumRenderTimeChange = Infinity;
  }
  unlockPageScroll();
}

function open2dFromEarth3d(){
  closeEarth3d();
  $$('.tabbtn').forEach(b=>b.classList.toggle('active', b.dataset.tab === 'radarscreen'));
  $$('.screen').forEach(s=>s.classList.remove('active'));
  $('#radarscreen')?.classList.add('active');
  state.activeTab = 'radarscreen';
  initMapIfNeeded();
  setTimeout(()=>state.map?.invalidateSize(), 120);
}

function initEarth3dViewer(){
  if(state.earth3d.viewer || !window.Cesium) return;
  const Cesium = window.Cesium;
  const viewer = new Cesium.Viewer('earth3dCanvas', {
    animation:false,
    timeline:false,
  ...
  viewer.scene.globe.maximumScreenSpaceError = state.earth3d.eco ? 6 : (likelyWeakDevice() ? 4 : 2);
  viewer.resolutionScale = state.earth3d.eco ? .58 : (likelyWeakDevice() ? .72 : 1);
  viewer.targetFrameRate = state.earth3d.eco ? 18 : (likelyWeakDevice() ? 24 : 45);
  viewer.imageryLayers.addImageryProvider(new Cesium.UrlTemplateImageryProvider({
    url:'https://{s}.basemaps.cartocdn.com/rastertiles/voyager_nolabels/{z}/{x}/{y}{r}.png',
    subdomains:['a','b','c','d'],
    maximumLevel: likelyWeakDevice() ? 7 : 9,
    credit:'OpenStreetMap, CARTO'
  }));
  state.earth3d.viewer = viewer;
  document.addEventListener('visibilitychange', handleEarth3dVisibility);
  window.addEventListener('orientationchange', ()=>setTimeout(()=>viewer.resize(), 250));
  window.addEventListener('resize', ()=>viewer.resize());
}

function handleEarth3dVisibility(){
  if(!state.earth3d.viewer) return;
  if(document.hidden) stopEarth3dPlayback();
  state.earth3d.viewer.scene.requestRenderMode = true;
}

function wireEarth3dControls(){
  if(state.earth3d.wired) return;
  state.earth3d.wired = true;
  $('#earth3dClose')?.addEventListener('click', closeEarth3d);
  $('#earth3dBack2d')?.addEventListener('click', open2dFromEarth3d);
  $('#earth3dFallback2d')?.addEventListener('click', open2dFromEarth3d);
  $('#earth3dLayer')?.addEventListener('change', async e=>{
    state.earth3d.layer = e.target.value;
    stopEarth3dPlayback();
    await refreshEarth3dFrames();
  });
  $('#earth3dLocate')?.addEventListener('click', async ()=>{
    const p = await getBrowserLocation();
    const loc = p || state.loc;
    flyEarth3dTo(loc.lat, loc.lon, 1800000);
  });
  $('#earth3dPlay')?.addEventListener('click', toggleEarth3dPlayback);
  $('#earth3dEco')?.addEventListener('change', e=>{
    state.earth3d.eco = e.target.checked;
    if(state.earth3d.viewer){
      state.earth3d.viewer.resolutionScale = state.earth3d.eco ? .58 : (likelyWeakDevice() ? .72 : 1);
      state.earth3d.viewer.targetFrameRate = state.earth3d.eco ? 18 : (likelyWeakDevice() ? 24 : 45);
      state.earth3d.viewer.scene.globe.maximumScreenSpaceError = state.earth3d.eco ? 6 : (likelyWeakDevice() ? 4 : 2);
      state.earth3d.viewer.scene.requestRender();
    }
  });
  $('#earth3dSearchInput')?.addEventListener('input', e=>{
    clearTimeout(state.earth3d.searchTimer);
    const q = e.target.value.trim();
    if(q.length < 2){ $('#earth3dSuggestions')?.classList.remove('show'); return; }
    state.earth3d.searchTimer = setTimeout(()=>searchEarth3dPlace(q), 300);
  });
}

async function refreshEarth3dFrames(){
  if(state.earth3d.layer === 'daynight'){
    state.earth3d.frames = [{time:Math.floor(Date.now()/1000), path:null}];
    state.earth3d.index = 0;
    removeEarth3dWeatherLayer();
    updateEarth3dTimeline();
    updateEarth3dLegend('Dag- en nachtgrens', 'Bron: CesiumJS zonpositie. Timestamp: nu. Resolutie: realtime belichting. Vertraging: geen externe weerlaag.');
    state.earth3d.viewer?.scene.requestRender();
    return;
  }
  if(!rainviewerMeta || !isFreshRadarFrame(latestRainviewerObservedFrame(rainviewerMeta))){
    await loadRadarFrames(true);
  }
  const frames = earth3dFrameSet();
  state.earth3d.frames = frames.sort((a,b)=>a.time-b.time);
  if(!state.earth3d.frames.length){
    removeEarth3dWeatherLayer();
    updateEarth3dTimeline();
    const layerLabel = state.earth3d.layer === 'satellite' ? 'Satelliet infrarood' : 'Neerslagradar';
    updateEarth3dLegend(layerLabel, 'Deze laag heeft momenteel geen live frames in de RainViewer metadata. Er wordt geen oud of fictief beeld getoond.');
    if($('#earth3dSubtitle')) $('#earth3dSubtitle').textContent = `${layerLabel} niet beschikbaar`;
    state.earth3d.viewer?.scene.requestRender();
    return;
  }
  const observed = state.earth3d.frames.filter(f=>!f.isNowcast);
  state.earth3d.index = Math.max(0, state.earth3d.layer === 'satellite' ? state.earth3d.frames.length - 1 : observed.length - 1);
  updateEarth3dTimeline();
  setEarth3dFrame(state.earth3d.index);
}

function earth3dFrameSet(){
  if(!rainviewerMeta) return [];
  if(state.earth3d.layer === 'satellite'){
    return (rainviewerMeta.satellite?.infrared || rainviewerMeta.satellite?.visible || []).map(f=>({...f}));
  }
  const past = rainviewerMeta.radar?.past || [];
  const nowcast = rainviewerMeta.radar?.nowcast || [];
  const cutoff = (Date.now()/1000) - 2*60*60;
  return past.filter(f=>f.time >= cutoff).concat(nowcast.map(f=>({...f,isNowcast:true})));
}

function earth3dTileUrl(frame){
  const host = rainviewerMeta?.host;
  if(!host || !frame?.path) return null;
  if(state.earth3d.layer === 'satellite') return `${host}${frame.path}/256/{z}/{x}/{y}/0/0_0.png?earth3d=${frame.time}`;
  return `${host}${frame.path}/256/{z}/{x}/{y}/${state.radar.scheme || 4}/1_1.png?earth3d=${frame.time}`;
}

function removeEarth3dWeatherLayer(){
  const viewer = state.earth3d.viewer;
  if(viewer && state.earth3d.imageryLayer){
    viewer.imageryLayers.remove(state.earth3d.imageryLayer, true);
  }
  state.earth3d.imageryLayer = null;
}

function setEarth3dFrame(index){
  const viewer = state.earth3d.viewer;
  const Cesium = window.Cesium;
  if(!viewer || !Cesium || !state.earth3d.frames.length) return;
  state.earth3d.index = Math.max(0, Math.min(index, state.earth3d.frames.length - 1));
  const frame = state.earth3d.frames[state.earth3d.index];
  removeEarth3dWeatherLayer();
  const url = earth3dTileUrl(frame);
  if(url){
    state.earth3d.imageryLayer = viewer.imageryLayers.addImageryProvider(new Cesium.UrlTemplateImageryProvider({
      url,
      maximumLevel: likelyWeakDevice() ? 7 : 9,
      credit:'RainViewer'
    }));
    state.earth3d.imageryLayer.alpha = state.radar.opacity ?? .9;
  }
  viewer.clock.currentTime = Cesium.JulianDate.fromDate(new Date(frame.time * 1000));
  updateEarth3dTimeline();
  updateEarth3dMetadata(frame);
  viewer.scene.requestRender();
}

function updateEarth3dTimeline(){
  const el = $('#earth3dTimeline');
  if(!el) return;
  if(!state.earth3d.frames.length){
    el.innerHTML = '<span class="subtle">Geen frames beschikbaar.</span>';
    return;
  }
  el.innerHTML = state.earth3d.frames.map((frame,i)=>{
    const label = new Date(frame.time*1000).toLocaleTimeString('nl-BE',{hour:'2-digit',minute:'2-digit'});
    return `<button class="earth3d-frame ${frame.isNowcast?'forecast':''} ${i===state.earth3d.index?'active':''}" type="button" aria-label="${label}${frame.isNowcast ? ' verwacht' : ' gemeten'}" data-i="${i}" title="${label}${frame.isNowcast ? ' verwacht' : ' gemeten'}"></button>`;
  }).join('');
  $$('.earth3d-frame', el).forEach(btn=>btn.addEventListener('click', ()=>{
    stopEarth3dPlayback();
    setEarth3dFrame(Number(btn.dataset.i));
  }));
}

function updateEarth3dMetadata(frame){
  const layerLabel = state.earth3d.layer === 'satellite' ? 'Satelliet infrarood' : 'Neerslagradar';
  const time = new Date(frame.time * 1000);
  const age = Math.round((Date.now()/1000 - frame.time) / 60);
  const delay = frame.isNowcast ? 'prognoseframe' : (age > RADAR_MAX_AGE_MINUTES ? 'mogelijk verouderd' : 'normaal');
  const subtitle = `${layerLabel} - ${time.toLocaleTimeString('nl-BE',{hour:'2-digit',minute:'2-digit'})}${frame.isNowcast ? ' verwacht' : ''}`;
  if($('#earth3dSubtitle')) $('#earth3dSubtitle').textContent = subtitle;
  updateEarth3dLegend(layerLabel, `Databron: RainViewer Weather Maps. Timestamp: ${time.toLocaleString('nl-BE')}. Leeftijd: ${Math.max(0,age)} min. Resolutie: 256px tegels, ${likelyWeakDevice() ? 'mobiel beperkt LOD' : 'standaard LOD'}. Vertraging: ${delay}.`);
}

function updateEarth3dLegend(title, text){
  const el = $('#earth3dLegend');
  if(el) el.innerHTML = `<b>${esc(title)}</b><span>${esc(text)}</span><br><span>Niet getoond zonder echte tilebron: temperatuur, wind, bliksem, luchtkwaliteit, sneeuw en tropische systemen.</span>`;
}

function toggleEarth3dPlayback(){
  if(state.earth3d.timer){ stopEarth3dPlayback(); return; }
  if(state.earth3d.frames.length < 2) return;
  $('#earth3dPlay').textContent = 'Pauze';
  const interval = Number($('#earth3dSpeed')?.value || 800);
  state.earth3d.timer = setInterval(()=>{
    let next = state.earth3d.index + 1;
    if(next >= state.earth3d.frames.length) next = 0;
    setEarth3dFrame(next);
  }, interval);
}

function stopEarth3dPlayback(){
  clearInterval(state.earth3d.timer);
  state.earth3d.timer = null;
  if($('#earth3dPlay')) $('#earth3dPlay').textContent = 'Afspelen';
}

function flyEarth3dTo(lat, lon, height=2200000){
  const viewer = state.earth3d.viewer;
  const Cesium = window.Cesium;
  if(!viewer || !Cesium) return;
  viewer.camera.flyTo({
    destination:Cesium.Cartesian3.fromDegrees(Number(lon), Number(lat), height),
    duration: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 1.2
  });
}

async function searchEarth3dPlace(q){
  const box = $('#earth3dSuggestions');
  if(!box) return;
  try{
    ...
    box.innerHTML = results.map((res,i)=>`<button type="button" data-i="${i}"><b>${esc(res.name)}</b><br><span>${esc([res.admin1,res.country].filter(Boolean).join(', '))}</span></button>`).join('');
    box.classList.add('show');
    $$('button', box).forEach(btn=>btn.addEventListener('click', ()=>{
      const res = results[Number(btn.dataset.i)];
      flyEarth3dTo(res.latitude, res.longitude, 1800000);
      box.classList.remove('show');
      $('#earth3dSearchInput').value = '';
    }));
  }catch(e){
    box.classList.remove('show');
  }
